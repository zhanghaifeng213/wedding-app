<template>
    <div class="form">
        ...
        <radio-group @change="radioChange2" class="group">
            <label class="radio" v-for="(item, index) in regionList" :key="index">
...
export default {
  ...
  data () {
    return {
      ...
      regionList: [
        {
          name: '开原(9.11)', value: '开原(9.11)', checked: true
        }, {
          name: '大连(9.14)', value: '大连(9.14)', checked: false
        }
      ],
      desc: '',
      name: '',
  ...
  methods: {
    ...
    radioChange2 (e) {
      ...
      that.list.forEach(item => {
        item.checked = item.name === that.region
      })
    },
    submit () {
      const that = this
